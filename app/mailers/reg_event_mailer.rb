class RegEventMailer < ActionMailer::Base
  ...
  def welcome_email(reg_event)
    ...
    if reg_event.email_cc_1.length > 0
      b_cc_1 = true
    end
    if reg_event.email_cc_2.length > 0
      b_cc_2 = true
    end
    ...
    if !b_cc_1 and !b_cc_2      
      ...
      mail(:to => reg_event.email, :subject => 'The Learning Counsel', :cc => [reg_event.email_cc_1]) do |format|
        format.html
      end    

    elsif !b_cc_1 and b_cc_2

      mail(:to => reg_event.email, :subject => 'The Learning Counsel', :cc => [reg_event.email_cc_2]) do |format|
        format.html
      end    

    elsif b_cc_1 and b_cc_2

      mail(:to => reg_event.email, :subject => 'The Learning Counsel', :cc => [reg_event.email_cc_1, reg_event.email_cc_2]) do |format|
        format.html
      end    
