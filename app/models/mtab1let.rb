...
class Mtab1let < ActiveRecord::Base
  ...
  def self.find_by_email(email)
    ...
    # self.find(1)
    #self.connection.execute_query("SELECT * FROM mtab1lets")
  ...
  def self.dbdelete
      self.connection.execute("DELETE FROM mtab1lets")
  end

  
  def self.dbclear
      self.connection.execute("ALTER SEQUENCE mtab1lets_id_seq RESTART WITH 1")
  end
