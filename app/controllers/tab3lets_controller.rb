class Tab3letsController < ApplicationController
...
#           
    ...
    if params[:search_by].nil? and params[:export].nil? and @@delete == false
      @tab3lets = Tab3let.order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
      @@tab3lets = @tab3lets
      
    elsif !params[:export].nil? and @@delete == false
      
      export = params[:export]
      @tab3lets = Tab3let.where(params[:export]).paginate(:per_page => 200, :page => params[:page])
      #@tab3lets = Tab3let.where(:id => export.id)
    elsif !params[:search_by].nil? and @@delete == false

      # @tab3lets = Tab3let.where(:id => '99')
      ...
      @tab3lets = Tab3let.search_by_sql(s).order(sort_column + " " + sort_direction).paginate(:per_page => 200, :page => params[:page])
  ...
  def export
    ...
    # @tab3lets = Tab3let.where(params[:export])
  ...
  def import
    ...
    file_dir = 'public/tab3let/' + file_name
    ...
    # CSV.foreach(file_dir, {headers: :first_row}) do |row|
      ...
      sql = "
        INSERT INTO tab3lets (#{sql_keys.join(', ')}, created_at, updated_at)
  ...
  def upload
    ...
    # @tab3lets = Tab3let.order(:id)
    # respond_to do |format|
      # format.html
      # format.csv { send_data @tab3lets.to_csv }
...
# 
  ...
  def show
    @tab3let = Tab3let.find(params[:id])    
  end
  ...
  def edit
    @tab3let = Tab3let.find_by_id(params[:id])    
  end
  ...
  def update
    ...
    if @tab3let.update_attributes(params[:tab3let])
      # redirect_to(:action => 'show', :saved_id => @archive.id)
      
      redirect_to(:action => 'show', :saved_id => @tab3let.id)
  ...
  def destroy
    ...
    if tab3let.delete
      @@delete = true
      redirect_to(tab3lets_index_path)
    else
  ...
  private
    ...
    def sort_column
      Tab3let.column_names.include?(params[:sort]) ? params[:sort] : "id"
    end
