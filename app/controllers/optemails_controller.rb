class OptemailsController < ApplicationController
  ...
  def create
    ...
    optemail = Optemail.new(optemail_params)

    if optemail.save
      session[:optemail_saved] = true
    else
      session[:optemail_saved] = false
    end

    redirect_to "/EmailPreferencesSaved"
  ...
  def edit
      ...
      # @id_sent = params[:id]
      # sql = "SELECT * FROM mtab1lets WHERE id=" + @id_sent
      # @optemail = Mtab1let.find(@id_sent)
      #@optemail = create_object(Optemail, sql)
  ...
  def update
    ...
    # if mtab1let.update_attributes(params[:optemail])
  ...
  private  
    ...
    def optemail_params
      params.require(:optemail).permit(
                                        :col_1,
                                        :col_2,
                                        :col_3,
                                        :col_4,
